...
int main() {
  ...
  arma::Col<double> Tk1p1 = Tm;
  arma::Col<double> Tk1m1 = Tm;
  arma::Col<double> Tk2p1 = Tm;
  arma::Col<double> Tk2m1 = Tm;
  arma::Col<double> Tk3p1 = Tm;
  arma::Col<double> Tk3m1 = Tm;

  // Iterating
  //iterate<double>(coarse,1,1.0e-12,1000,Tc,k1,k2,k3);
  iterate<double>(medium,1,1.0e-12,1000,Tm,k1,k2,k3);
  iterate<double>(medium,1,1.0e-12,1000,Tk1p1,k1+dk1,k2,k3);
  iterate<double>(medium,1,1.0e-12,1000,Tk1m1,k1-dk1,k2,k3);
  iterate<double>(medium,1,1.0e-12,1000,Tk2p1,k1,k2+dk2,k3);
  iterate<double>(medium,1,1.0e-12,1000,Tk2m1,k1,k2-dk2,k3);
  iterate<double>(medium,1,1.0e-12,1000,Tk3p1,k1,k2,k3+dk3);
  iterate<double>(medium,1,1.0e-12,1000,Tk3m1,k1,k2,k3-dk3);
  //iterate<double>(fine,  1,1.0e-12,1000,Tf,k1,k2,k3);

  // Computing sensitivities
  arma::Col<double> sens_k1 = (Tk1p1 - Tk1m1)/(2.0*dk1);
  arma::Col<double> sens_k2 = (Tk2p1 - Tk2m1)/(2.0*dk2);
  arma::Col<double> sens_k3 = (Tk3p1 - Tk3m1)/(2.0*dk3);

  // Writing sensitivities to file
  std::vector<double> nodes = medium.get_nodes();
  std::ofstream sensfile("fdm.dat");
