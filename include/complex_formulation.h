 ...
 * The main difference between this header and formulation.h is a 
 * modification to allow for <=, >=, <, > comparisons of the real
...
A conductivity(A T, A k1, A k2, A k3) {
  ...
  A T2 = 50.0;
  A T3 = 100.0;
  if ((T1.real()<=T.real())&&(T.real()<=T2.real())) {
    return k1*(1.0 - (T-T1)/(T2-T1)) + k2*((T-T1)/(T2-T1));
  }
  else if ((T2.real()<T.real())&&(T.real()<=T3.real())) {
...
void sample_integrands(const element<T>& elem, const T xi, const std::vector<T>& nodes, const arma::Col<U>& temperature, const U k1, const U k2, const U k3, arma::Mat<U>& K_local, arma::Col<U>& F_local) {
  ...
  for (int i=0; i<2; ++i) {
    ...
    for (int j=0; j<2; ++j) {
      ...
      K_local(i,j) = conductivity<U>(Tval,k1,k2,k3)*dN_i*dN_j*detJ;
    }
    F_local(i) = (T) 0;
  }
